	  ...
	  window.onload = function() {
        ...
        eSelect.onchange = function() {
            if(eSelect.selectedIndex > 0) {
		        document.getElementById('app_id').value = [ <?php foreach ($app_list as $id => $data) echo "'$id', " ?> ][eSelect.selectedIndex-1];
        		document.getElementById('app_id_ipad').value = [ <?php foreach ($app_list as $id => $data) echo "'", $data['appid_ipad'], "', " ?> ][eSelect.selectedIndex-1];
		        document.getElementById('app_affiliate').value = [ <?php foreach ($app_list as $id => $data) echo "'", $data['affiliate_data'], "', " ?> ][eSelect.selectedIndex-1];
		        document.getElementById('app_argument').value = [ <?php foreach ($app_list as $id => $data) echo "'", $data['app_argument'], "', " ?> ][eSelect.selectedIndex-1];
            }
        }
      }   
      </script>
